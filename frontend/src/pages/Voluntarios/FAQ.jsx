      ...
      <main className="mx-auto max-w-6xl px-6 py-10 sm:px-12">
        <div className="space-y-6">
          {faqs.map((f, i) => (
            ...
            >
              <summary
                className="cursor-pointer list-none text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 rounded-md py-2"
                aria-expanded="false"
              >
                <div className="flex items-center justify-between gap-6">
                  <h3 className="text-base font-bold text-slate-900">{f.q}</h3>
                  <span className="shrink-0 rounded-full border border-violet-200 px-3 py-1 text-xs font-semibold text-violet-700 transition group-open:bg-violet-700 group-open:text-white">
                    Abrir
                  </span>
                </div>
              </summary>
              <p className="mt-4 text-sm text-slate-600">{f.a}</p>
            </details>
          ))}
        </div>

        <div className="mt-10 flex items-center gap-3">
          <span className="text-sm text-slate-600">Não encontrou sua dúvida?</span>
          <Link
